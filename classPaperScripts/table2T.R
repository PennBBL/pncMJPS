...
for(q in seq(1, length(foldsToLoop))){
    ...
    optLam <- cv.glmnet(y=as.vector(male.data$usageBin[-index]), x=as.matrix(male.data[-index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), alpha=0, family="binomial", parallel=F)
    lasModel2 <- glmnet(y=as.vector(male.data$usageBin[-index]), x=as.matrix(male.data[-index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), alpha=0, lambda=optLam$lambda.min)
    
    cvPredVals[index] <- predict(lasModel2, newx=as.matrix(male.data[index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), type='response')
...
for(q in seq(1, length(foldsToLoop))){
    index <- foldsToLoop[[q]]
    # Now do the real labels
    optLam <- cv.glmnet(y=as.vector(female.data$usageBin[-index]), x=as.matrix(female.data[-index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), alpha=0, family="binomial", parallel=F)
    lasModel2 <- glmnet(y=as.vector(female.data$usageBin[-index]), x=as.matrix(female.data[-index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), alpha=0, lambda=optLam$lambda.min)
    
    cvPredVals[index] <- predict(lasModel2, newx=as.matrix(female.data[index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), type='response')
