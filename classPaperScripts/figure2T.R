...
male.data[,1015:1032] <- scale(male.data[,1015:1032])
...
female.data[,1015:1032] <- scale(female.data[,1015:1032])

## Now prepare our vaues for FA in the labels
outVals <- NULL
allVals <- names(male.data)[1015:1032]
inVals <- allVals
for(q in inVals){
    tmpVal <- summarySE(male.data, measurevar=q, groupvars='marcat', na.rm=T)
    colnames(tmpVal)[3] <- 'mean'
    q <- gsub(pattern="dti_dtitk_jhutract_fa_", replacement="", x=q)
    tmpVal$ROI <- q
...
png('figure2-marcatMeanValuesTract.png', height=12, width=20, units='in', res=300)
outPlot
dev.off()
q()
