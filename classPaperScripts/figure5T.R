...
img.data <- read.csv('../data/imagingDataAll2.csv')
...
allAUCM <- foreach(z=seq(1,10), .combine=rbind) %dopar%{
    ...
    male.data <- male.data.all.m[complete.cases(male.data.all.m[,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]),]
    ...
    for(q in seq(1, length(foldsToLoop))){
        ...
        optLam <- cv.glmnet(y=as.vector(male.data$usageBin[-index]), x=as.matrix(male.data[-index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), alpha=0, family="binomial", parallel=T)
        lasModel1 <- glmnet(y=as.vector(male.data$usageBin[-index]), x=as.matrix(male.data[-index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), alpha=0, lambda=optLam$lambda.min)
        
        # Now do the real labels
        optLam <- cv.glmnet(y=as.vector(male.data$usageBinOrig[-index]), x=as.matrix(male.data[-index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), alpha=0, family="binomial", parallel=T)
        lasModel2 <- glmnet(y=as.vector(male.data$usageBinOrig[-index]), x=as.matrix(male.data[-index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), alpha=0, lambda=optLam$lambda.min)
        
        cvPredValsReal[index] <- predict(lasModel2, newx=as.matrix(male.data[index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), type='response')
        cvPredVals[index] <- predict(lasModel1, newx=as.matrix(male.data[index,c(grep('dti_dtitk_jhutract_fa', names(male.data)))]), type='response')
...
allAUCF <- foreach(z=seq(1,10), .combine=rbind) %dopar%{
    ...
    # Now lets see how well we can build our model in a cross validated fashion
    female.data <- female.data.all.m[complete.cases(female.data.all.m[,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]),]
    ...
    for(q in seq(1, length(foldsToLoop))){
        index <- foldsToLoop[[q]]
        # build a ridge model with the fake data
        optLam <- cv.glmnet(y=as.vector(female.data$usageBin[-index]), x=as.matrix(female.data[-index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), alpha=0, family="binomial", parallel=T)
        lasModel1 <- glmnet(y=as.vector(female.data$usageBin[-index]), x=as.matrix(female.data[-index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), alpha=0, lambda=optLam$lambda.min)
        
        # Now do the real labels
        optLam <- cv.glmnet(y=as.vector(female.data$usageBinOrig[-index]), x=as.matrix(female.data[-index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), alpha=0, family="binomial", parallel=T)
        lasModel2 <- glmnet(y=as.vector(female.data$usageBinOrig[-index]), x=as.matrix(female.data[-index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), alpha=0, lambda=optLam$lambda.min)
        
        cvPredValsReal[index] <- predict(lasModel2, newx=as.matrix(female.data[index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), type='response')
        cvPredVals[index] <- predict(lasModel1, newx=as.matrix(female.data[index,c(grep('dti_dtitk_jhutract_fa', names(female.data)))]), type='response')
...
allAUCM <- foreach(z=seq(1,10), .combine=rbind, .errorhandling='remove') %dopar%{
    # Load library(s)
    install_load('glmnet', 'caret', 'pROC', 'useful')
    # Create a random binary outcome
    male.data.all.m$usageBin <- rbinom(dim(male.data.all.m)[1], 1, propValueMale)
...
allAUCF <- foreach(z=seq(1,10), .combine=rbind, .errorhandling='remove') %dopar%{
    # Load library(s)
    install_load('glmnet', 'caret', 'pROC', 'useful')
    # Create a random binary outcome
